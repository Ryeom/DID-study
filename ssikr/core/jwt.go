...
// vp는 vc랑 동일한 검증인데 검증됐다해서 클레임 대놓고 쓰는게아니라
// vc를 전부 열어서 issuer가 검증되는지를 하나하나 체크해야한닥 (만료, 상태변경 등...)
...
//
//			for idx, vcToken := range vcList { // vc의 verify를 하나하나 다해야대! 값을 어떻게 할지는 서비스 문제이다.!
